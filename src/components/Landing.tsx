      ...
      <div className="relative">
        ...
        <div className="relative">
          <header className="flex items-center justify-between px-4 py-5 sm:px-10">
            ...
            <Link
              to="/auth"
              className="rounded bg-primary px-5 py-2 font-semibold text-primary-foreground transition hover:bg-primary/85"
            >
              Sign In
            </Link>
          </header>
          ...
          <div className="mx-auto max-w-3xl px-4 py-24 text-center sm:py-36">
            <h1 className="text-balance text-4xl font-extrabold leading-tight sm:text-6xl">
              Unlimited movies, TV shows & cartoons.
            </h1>
            <p className="mt-4 text-xl">Watch anywhere. Cancel anytime.</p>
            <p className="mt-6 text-base text-foreground/80">
              Ready to watch? Create your account to start streaming.
              <br />
              <span className="text-sm text-muted-foreground">Available in Trinidad & Tobago.</span>
            </p>
            <Link
              to="/auth"
              search={{ mode: "signup" }}
              className="mt-6 inline-block rounded-md bg-primary px-8 py-4 text-lg font-bold text-primary-foreground shadow-[var(--shadow-red)] transition hover:bg-primary/85"
            >
              Get Started
            </Link>
          </div>
      ...
      <section className="mx-auto max-w-4xl px-4 pb-24">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          {Object.values(PLANS).map((plan) => (
            <div
              key={plan.id}
              className="flex flex-col rounded-xl border border-border bg-card p-7 transition hover:border-primary"
            >
              ...
              <ul className="mt-5 space-y-2 text-sm">
                <li className="flex items-center gap-2">
                  <Check className="h-4 w-4 text-primary" /> {plan.screens} screens at the same time
                </li>
                <li className="flex items-center gap-2">
                  <Check className="h-4 w-4 text-primary" /> {plan.quality}
                </li>
                <li className="flex items-center gap-2">
                  <Check className="h-4 w-4 text-primary" /> Unlimited movies, TV & cartoons
                </li>
              </ul>
              <Link
                to="/auth"
                search={{ mode: "signup" }}
                className="mt-7 rounded-md bg-primary py-3 text-center font-semibold text-primary-foreground transition hover:bg-primary/85"
              >
                Choose {plan.name}
              </Link>
            </div>
