      ...
      <div className="flex items-center justify-between px-4 py-3 sm:px-8">
        ...
        <div className="flex items-center gap-4">
                ...
                <div className="absolute right-0 top-full mt-2 w-64 rounded-xl border border-border bg-popover shadow-2xl z-50 overflow-hidden">
                  ...
                  {profiles.filter((p) => p.id !== activeProfile?.id).length > 0 && (
                    <div className="border-b border-border py-1.5">
                      <p className="px-5 pt-1.5 pb-1 text-xs text-muted-foreground">Switch profile</p>
                      {profiles
                        ...
                        .map((p) => (
                          <button
                            ...
                            className="flex w-full items-center gap-3 px-5 py-3 text-sm hover:bg-accent focus-visible:outline-none focus-visible:bg-accent"
                          >
                            <span
                              className="flex h-8 w-8 items-center justify-center rounded text-sm font-bold text-white shrink-0"
                  ...
                  <div className="py-1.5">
                    <Link
                      ...
                      className="block px-5 py-3.5 text-sm font-medium hover:bg-accent focus-visible:outline-none focus-visible:bg-accent"
                    >
                      All Profiles
                    </Link>
                    <Link
                    ...
                    {isAdmin && (
                      <Link
                        to="/admin"
                        onClick={() => setProfileOpen(false)}
                        className="block px-5 py-3.5 text-sm font-bold text-primary hover:bg-accent focus-visible:outline-none focus-visible:bg-accent"
                      >
                        Admin Panel
                      </Link>
                    )}
                    ...
                    <button
                      ...
                      className="block w-full px-5 py-3.5 text-left text-sm font-semibold text-destructive hover:bg-accent focus-visible:outline-none focus-visible:bg-accent"
                    >
                      Sign Out
                    </button>
                  </div>
          ...
          <div className="relative md:hidden">
            ...
            {mobileOpen && (
              <nav className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-border bg-popover py-1.5 shadow-2xl z-50 overflow-hidden">
                {LINKS.map((l) => (
                  <NavLink
                    key={l.to}
                    to={l.to}
                    ...
                    className={({ isActive }) =>
                      `block px-5 py-3.5 text-sm font-medium hover:bg-accent ${
